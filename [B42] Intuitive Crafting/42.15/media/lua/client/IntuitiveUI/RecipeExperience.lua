...
local xpAwardCache = setmetatable({}, { __mode = "k" })
local function getXPAwards(recipe)
    local cached = xpAwardCache[recipe]
    if cached then return cached end

    local result = {}
    local byPerkId = {}

    local bodies = recipe.getLoadedScriptBodies and recipe:getLoadedScriptBodies()
    if bodies then
        for i = 1, bodies:size() - 1, 2 do
            local bodyStr = tostring(bodies:get(i))
            for xpLine in bodyStr:gmatch("[^\n\r]+") do
                local xpStr = xpLine:find("xpAward", 1, true) and xpLine:match("xpAward%s*=%s*(.+)")
                if xpStr then
                    for perkName, amountStr in xpStr:gmatch("([%w_%.]+)%s*:%s*([%d%.]+)") do
                        local amount = tonumber(amountStr) or 0
                        local entry = byPerkId[perkName]
                        if entry then
                            entry.amount = entry.amount + amount
                        else
                            local perkObj = Perks and Perks[perkName]
                            if perkObj then
                                entry = { perk = perkObj, amount = amount }
                                byPerkId[perkName] = entry
...
function ISRecipeScrollingListBox:doDrawNode(y, item, _alt)
    ...
    if craftRecipe then
        ...
        local xpAwards = getXPAwards(craftRecipe)
        
        if not craftRecipe:getTooltip() and #xpAwards == 0 and craftRecipe:getRequiredSkillCount() == 0 then
        ...
        if #xpAwards > 0 then
            ...
            for _, xpEntry in ipairs(xpAwards) do
                local skillPerk = xpEntry.perk
                local xpAmount  = xpEntry.amount
