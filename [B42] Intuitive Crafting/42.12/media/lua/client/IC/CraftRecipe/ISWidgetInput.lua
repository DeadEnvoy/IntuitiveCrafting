...
function ISWidgetInput:createScriptValues(_script, isSecondary)
    ...
    table.icon.tooltipUI.maxLineWidth = 1000
...
function ISWidgetInput:updateValues()
    ...
    if self.logic and self.interactiveMode and self.inputScript:getResourceType()==ResourceType.Item then
        ...
        local satisfiedAmount = 0
        if self.primary.inputItem then
            local itemType = self.primary.inputItem:getFullName()
            local containers = self.logic:getContainers()
            for i=0, containers:size()-1 do
                satisfiedAmount = satisfiedAmount + containers:get(i):getCountTypeRecurse(itemType)
            end
        end
...
function ISWidgetInput:new (x, y, width, height, player, logic, inputScript)
