...
function ISWidgetIngredientsInputs:createChildren()
    ...
    self.toolsLabel = ISXuiSkin.build(self.xuiSkin, "S_NeedsAStyle", ISLabel, 0, 0, fontHeight, getText("IGUI_CraftingWindow_Tools") or "Tools", 1.0, 1.0, 1.0, 1, UIFont.Small, true);
    self.toolsLabel:initialise();
    self.toolsLabel:instantiate();
    self:addChild(self.toolsLabel);
    
    self.materialsLabel = ISXuiSkin.build(self.xuiSkin, "S_NeedsAStyle", ISLabel, 0, 0, fontHeight, getText("IGUI_CraftingWindow_Requires") or "Materials", 1.0, 1.0, 1.0, 1, UIFont.Small, true);
...
function ISWidgetIngredientsInputs:calculateLayout(_preferredWidth, _preferredHeight)
    ...
    local baseColumns = 4;
    local toolRowsBase = #self.tools > 0 and math.ceil(#self.tools / baseColumns) or 0;
    local materialRowsBase = #self.materials > 0 and math.ceil(#self.materials / baseColumns) or 0;
    local totalRowsBase = toolRowsBase + materialRowsBase;
    
    local toolCols = 4;
    local materialCols = 4;
    
    if totalRowsBase == 4 then
        toolCols = 5; materialCols = 5;
    elseif totalRowsBase >= 5 then
        toolCols = 6; materialCols = 6;
    end
    
    if #self.tools >= 5 and #self.tools ~= 6 then
        toolCols = 5;
    elseif #self.tools == 6 then
        toolCols = 6;
    end
    
    if #self.materials >= 5 and #self.materials ~= 6 then
        materialCols = 5;
    elseif #self.materials == 6 then
        materialCols = 6;
    end
